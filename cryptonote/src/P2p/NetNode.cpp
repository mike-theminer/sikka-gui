...
const char* value = tmp.c_str();
   ...
   const std::string tmp = boost::uuids::to_string(arg.node_data.network_id); 
    const char* value = tmp.c_str();
    
	printf("im here request %s %d %ld %d\n",value,arg.node_data.version, arg.node_data.local_time, arg.node_data.my_port  );
