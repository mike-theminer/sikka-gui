...
bool Currency::generateGenesisBlock() {
  ...
  std::string genesisCoinbaseTxHex = "010a01ff00018080aace938c090243d9c15d049ebdee2bab711cfa9eb9372708b0d8f1c2820716c4e640010bd5ef210193f52822f6d6a4605759fd941f09c4438876708dd1b422c2296c2e51e70b0665";
...
*/		
//	printf("call me difficultyBlocksCount baby %d\n",height);
	uint32_t m_difficultyLag_temp = 0;
	uint32_t m_difficultyWindow_temp = 17;
	uint32_t m_difficultyWindowV2_temp = 720 ;
	uint32_t m_difficultyLagV2_temp = 15;  
	if(height >= 40000){
		return m_difficultyWindow_temp + m_difficultyLag_temp;
		
		}
		else{
			return m_difficultyWindowV2_temp + m_difficultyLagV2_temp;
...
size_t Currency::difficultyLagByBlockVersion(uint8_t blockMajorVersion) const {
 ...
 //difficultyWindowByBlockVersion(blockMajorVersion) + difficultyLagByBlockVersion(blockMajorVersion);
...
/*
  ...
  */
  ...
  assert(m_emissionSpeedFactor > 0 && m_emissionSpeedFactor <= 8 * sizeof(uint64_t));
 uint64_t m_genesisBlockReward = 40000000000000;
 uint64_t m_tailEmissionReward = 0;
...
//printf("genesis reward %ld ", m_genesisBlockReward);
...
Difficulty Currency::nextDifficulty(uint8_t blockMajorVersion, std::vector<uint64_t> timestamps,
  std::vector<Difficulty> cumulativeDifficulties) const {
	...
	//printf("inside nextDifficulty %d",blockMajorVersion);
/*	if (blockMajorVersion >= BLOCK_MAJOR_VERSION_3) {
...
std::vector<uint64_t> cumulativeDifficulties_o(cumulativeDifficulties);
  ...
  size_t c_difficultyWindow = 0;
  
  size_t m_difficultyWindowV2 = 720;
   size_t m_difficultyWindowV1 = 720;
   
    if (blockMajorVersion >= BLOCK_MAJOR_VERSION_3) {
    c_difficultyWindow= 17;
  } else if (blockMajorVersion == BLOCK_MAJOR_VERSION_2) {
    c_difficultyWindow =  m_difficultyWindowV2;
  } else {
    c_difficultyWindow =  m_difficultyWindowV1;
  }
  
size_t m_difficultyCutV1 = 60;
size_t m_difficultyCutV2 = 60;
  size_t c_difficultyCut = 0 ;//Currency::difficultyCutByBlockVersion(version);
    if (blockMajorVersion >= BLOCK_MAJOR_VERSION_3) {
    c_difficultyCut = 0;
  } else if (blockMajorVersion == BLOCK_MAJOR_VERSION_2) {
    c_difficultyCut =  m_difficultyCutV2;
  } else {
    c_difficultyCut =  m_difficultyCutV1;
  }
...
/*
	 printf("\n");
 for(int i=0; i<cumulativeDifficulties.size(); ++i)
  std::cout << cumulativeDifficulties[i] << ' ';
	printf("\n");
...
//~ printf ("\nc_difficultyWindow %ld, c_difficultyCut %ld length%ld\n",c_difficultyWindow, c_difficultyCut, length);
  ...
  //printf("\n mul128  %ld %ld fuck me\n",totalWork,m_difficultyTarget );
  if (high != 0 || std::numeric_limits<uint64_t>::max() - low < (timeSpan - 1)) {
	  
    return 0;
  }

	// printf ("\ncutend %ld, cutbegin %ld \n",cutEnd, cutBegin);
	
  uint8_t c_zawyDifficultyBlockVersion = 3;
  uint8_t m_zawyDifficultyV2 = 0;
  if (m_zawyDifficultyV2) {
    c_zawyDifficultyBlockVersion = 2;
  }
  
  if (blockMajorVersion >= c_zawyDifficultyBlockVersion && c_zawyDifficultyBlockVersion) {
    if (high != 0) {
		printf("\nc_zawyDifficultyBlockVersion \n");
      return 0;
    }
    //printf("\nc_zawyDifficultyBlockVersion low %ld  time %ld\n",low, timeSpan);
    uint64_t nextDiffZ = low / timeSpan;

    return nextDiffZ;
  }
 
	uint32_t m_zawyDifficultyBlockIndex = 0;
/*	
...
uint32_t m_buggedZawyDifficultyBlockIndex = 0;
/*
