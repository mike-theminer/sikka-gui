...
bool WalletApplication::init() {
	fprintf(stderr, "in app,init 111 \n");
  ...
  fprintf(stderr, "in Settings instance 111 \n");
  ...
  fprintf(stderr, "in app,init %s \n",BYTECOIN_URI_SCHEME_NAME);
...
fprintf(stderr, "m_applicationEventHandler\n ");
  ...
  fprintf(stderr, "isRunMinimizedEnabled\n ");
  ...
  m_cryptoNoteAdapter = new CryptoNoteAdapter(Settings::instance().getDataDir(), Settings::instance().isTestnet(),
    ...
    fprintf(stderr, "m_cryptoNoteAdapter\n ");
...
bool WalletApplication::initCryptoNoteAdapter() {
	fprintf(stderr, "initCryptoNoteAdapter 111 \n");
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
