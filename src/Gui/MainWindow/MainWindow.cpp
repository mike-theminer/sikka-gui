...
const char COMMUNITY_FORUM_URL[] = "https://sikkapay.slack.com/";
const char REPORT_ISSUE_URL[] = "https://bitcointalk.org/index.php?topic=2197967.0";
...
MainWindow::MainWindow(ICryptoNoteAdapter* _cryptoNoteAdapter, IAddressBookManager* _addressBookManager,
  ...
  QList<IWalletUiItem*> uiItems;
  uiItems << m_ui->m_noWalletFrame << m_ui->m_overviewFrame << m_ui->m_sendFrame << m_ui->m_transactionsFrame <<
    m_ui->m_addressBookFrame << m_ui->m_miningFrame << m_ui->statusBar;
...
void MainWindow::copyAddress() {
  printf("copy address %d \n",WalletStateModel::COLUMN_ADDRESS);
  QApplication::clipboard()->setText("fuck");
  printf("copyAddress %s \n",m_walletStateModel->index(0, WalletStateModel::COLUMN_ADDRESS).data().toString());
